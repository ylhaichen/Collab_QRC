...
if [[ "${available}" != true ]]; then
  blocker="Dynamic-LIO source not found at ${SRC_DIR}; run scripts/setup/fetch_slam_backends.sh"
else
  case "${DEPLOYMENT_MODE}" in
    sim_hybrid_ros1_slam_ros2_nav|sim_ros2)
      if [[ "${docker_cli_available}" == true && "${docker_daemon_available}" == true ]]; then
        ...
        blocker="Dynamic-LIO source exists, but sim hybrid build requires Docker daemon access for ROS1/Noetic catkin"
      fi
      ;;
    real_hybrid_ros1_slam_ros2_nav|real_ros1_only_experimental)
      if [[ "${native_catkin_available}" == true ]]; then
        ...
        blocker="Dynamic-LIO source exists, but real hybrid build requires ROS1 Noetic, catkin_make, and rospack on the onboard host"
      fi
      ;;
    *)
...
if [[ -f "${SIM_WS}/devel/setup.bash" || -d "${SRC_DIR}/devel" || -d "${SRC_DIR}/install" ]]; then
  ...
  blocker="Dynamic-LIO source is buildable for ${DEPLOYMENT_MODE}, but no ROS1 filtering runtime artifact was found"
fi
